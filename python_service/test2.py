...
# Batch/process images from a folder instead of webcam
...
image_paths = sorted([p for p in input_dir.glob("**/*") if p.suffix.lower() in ('.jpg', '.jpeg', '.png', '.bmp')])
if not image_paths:
    print(f"No images found in {input_dir}. Place test images there or pass folder as argv.")
    sys.exit(0)
...
for image_path in image_paths:
    ...
    if not result.face_landmarks:
        out_no_face = output_dir / f"noface_{image_path.name}"
        cv2.imwrite(str(out_no_face), frame)
        print(f"No face detected in {image_path.name}; saved original to {out_no_face.name}")
        continue
    ...
    if face_rgb.size == 0 or face_bgr.size == 0:
        out_err = output_dir / f"badcrop_{image_path.name}"
        cv2.imwrite(str(out_err), frame)
        print(f"Invalid face crop for {image_path.name}; saved original to {out_err.name}")
        continue
...
# ---- NMS ----
    ...
    detections = non_max_suppression(pred, conf_thres=0.05, iou_thres=0.45)[0]
    ...
    print(f"Processed {image_path.name} -> {out_path}")
